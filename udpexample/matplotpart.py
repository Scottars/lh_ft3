...
class MyDynamicMydynamcifigure(MyFigure):
    """动态画布：每秒自动更新，更换一条折线。"""
    def __init__(self, *args, **kwargs):
        MyFigure.__init__(self, *args, **kwargs)
...
class MainDialogImgBW(QDialog,Ui_Dialog):
    def __init__(self):
        ...
        self.groupBox = QGridLayout(self.groupBox)  # 继承容器groupBox
        self.groupBox.addWidget(self.F,0,1)
        ...
        # self.plotdynamic()
        # 通过按键触发绘制图像
        self.pushButton.clicked.connect(self.plotdynamic)
        self.pushButton_2.clicked.connect(self.stopplotdynamic)
    ...
    def plotother(self):
        ...
        self.groupBox.addWidget(F1, 0, 2)

    def plotdynamic(self):
        ## 调用figure下面的add_subplot方法，类似于matplotlib.pyplot下面的subplot方法
        self.F2 = MyDynamicMydynamcifigure(width=5, height=4, dpi=100)
        ...
        # F1.axes2.plot(x, y)
        # F1.axes2.set_title("line")
        self.groupBox.addWidget(self.F2,0,3)  #ps 这个内部的参数是指的是 widget 是第几行第几个,计数从0 开始
    def stopplotdynamic(self):
        # self.groupBox.removeWidget(self.groupBox)
        ...
        # self.groupBox.destroyed(self.F2)
